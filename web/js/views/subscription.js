...
export function renderSubscription(app) {
  ...
  function syncBtn() {
    toggleBtn.innerHTML = (state.me.beta ? icon('check', 16) : icon('sparkle', 16)) + '<span>' + (state.me.beta ? 'Beta ativado — desativar' : 'Ativar Orbit+ (beta grátis)') + '</span>';
    statusPill.textContent = state.me.beta ? 'ATIVO' : 'INATIVO';
  }
  syncBtn();
  toggleBtn.addEventListener('click', async () => {
    try { const { beta } = await api.post('/api/subscription', { beta: !state.me.beta }); state.me.beta = beta; syncBtn(); if (beta) { sfx.success(); toast('Orbit+ ativado (beta)', 'success'); } else toast('Orbit+ desativado', 'info'); app.renderSidebar(); }
    catch (e) { toast(e.message, 'error'); }
  });

  inner.append(
    h('div', { class: 'row', style: { justifyContent: 'space-between', alignItems: 'center' } },
      h('h1', { class: 'page-title' }, h('span', { class: 'ico', html: icon('sparkle', 30) }), ' Orbit+'),
      statusPill),
    ...
    h('div', { class: 'section-card' },
      ...
      h('div', { style: { marginTop: '12px' } },
        benefit('sparkle', 'Selo premium no perfil', 'Apareça com o selo Orbit+ ao lado do seu nome.'),
    ...
    h('div', { class: 'section-card center', style: { flexDirection: 'column', gap: '10px', padding: '34px' } },
      ...
      h('p', { class: 'muted', style: { textAlign: 'center', maxWidth: '420px' } }, 'O pagamento da assinatura ainda é beta. Por enquanto, o Orbit+ é gratuito e o checkout chega em breve.'),
